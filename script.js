document.addEventListener("DOMContentLoaded", () => {
 ...
 // 追加項目も検索対象に含める
 const entryYear = stu.entryYear ?? "";
 const admissionEnglish = stu.admissionEnglish ?? "";
 const selectionFormat = stu.selectionFormat ?? "";
 const timeToPass = stu.timeToPass ?? "";
 const previousEducation = stu.previousEducation ?? "";
 const workYears = stu.workYears ?? "";

 const onlineConversation = Array.isArray(stu.onlineConversation)
 ? stu.onlineConversation.map((x) => `${x.label || ""} ${x.url || ""}`).join(" ")
 : "";

 const studyMaterials = Array.isArray(stu.studyMaterials)
 ? stu.studyMaterials.map((x) => `${x.label || ""} ${x.url || ""}`).join(" ")
 : "";
 ...
 const v = toStr(value);
 if (!v) return "";
 return `<div class="metaRow"><span class="metaK">${esc(label)}：</span><span class="metaV">${esc(v)}</span></div>`;
 }

 function metaRowNumHtml(label, value, suffix = "") {
 const n = toNumOrBlank(value);
 if (n === "") return "";
 const v = `${n}${suffix}`;
 return `<div class="metaRow"><span class="metaK">${esc(label)}：</span><span class="metaV">${esc(v)}</span></div>`;
 }

 function renderResourceSection(title, items) {
 if (!Array.isArray(items) || !items.length) return "";

 const pills = items
 .filter((x) => x && (x.label || x.url))
 .map((x) => {
 const label = toStr(x.label) || "リンク";
 const ready = isLinkReady(x);
 const url = toStr(x.url) || "#";
 ...
 return `
 <div class="materialBox" aria-label="${esc(title)}">
 <p class="materialTitle">${esc(title)}</p>
 <div class="materialList">${pills}</div>
 </div>
 `;
 }
 ...
 metaRowHtml("地域", stu.region),
 metaRowHtml("分野", stu.course),
 metaRowHtml("専攻", stu.major),
 metaRowHtml("入学年", stu.entryYear),
 metaRowHtml("学年", stu.year),
 metaRowHtml("合格時英語力", stu.admissionEnglish),
 metaRowHtml("選考形式", stu.selectionFormat),
 metaRowHtml("合格までの期間", stu.timeToPass),
 metaRowHtml("最終学歴", stu.previousEducation),
 metaRowNumHtml("社会人経験", stu.workYears, "年"),
 metaRowHtml("語学", stu.language),
 metaRowHtml("面談", stu.meeting),
 ]
 .filter(Boolean)
 .join("");

 // アフィリエイト導線：専用ブロック
 const onlineConvSection = renderResourceSection("合格までに使ったオンライン英会話", stu.onlineConversation);
 const materialsSection = renderResourceSection("合格までに使った教材", stu.studyMaterials);
 ...
 ${onlineConvSection || materialsSection ? `<div class="materialWrap">${onlineConvSection}${materialsSection}</div>` : ""}
 ...
 // 追加項目も候補に含める
 if (s.entryYear) set.add(String(s.entryYear));
 if (s.admissionEnglish) set.add(String(s.admissionEnglish));
 if (s.selectionFormat) set.add(String(s.selectionFormat));
 if (s.previousEducation) set.add(String(s.previousEducation));
 if (s.timeToPass) set.add(String(s.timeToPass));

 if (Array.isArray(s.onlineConversation)) {
 s.onlineConversation.forEach((x) => x?.label && set.add(String(x.label)));
 }
 if (Array.isArray(s.studyMaterials)) {
 s.studyMaterials.forEach((x) => x?.label && set.add(String(x.label)));
 }
 });
